"""
Pricing Intelligence Agent for AWS Bedrock AgentCore Runtime (Strands Framework)
...
def get_price_guardrails(category: str = None) -> Dict:
    ...
    return {'guardrails': guardrails, 'category': category or 'All Categories', 'timestamp': datetime.now().isoformat()}
...
def calculate_recommended_price_range(
    ...
    """
    Calculate recommended price range with margin guardrails for a product or category.

    Args:
        sku: Product SKU to analyze (optional if category provided)
        category: Product category to analyze (optional if sku provided)
        current_price: Override current price (optional)
        cost_price: Known cost price (optional, estimated if not provided)

    Returns:
        Dict with current_price, price_range (min/optimal/max), recommendation action,
        suggested_price, price_change_percent, and guardrails.
    """
    try:
        if sku:
...
def get_competitor_price_comparison(sku: str = None, category: str = None) -> Dict:
    ...
    Args:
        sku: Product SKU to compare (optional if category provided)
        category: Product category to compare (optional if sku provided)

    Returns:
        Dict with our_price, avg_competitor_price, price_difference_pct,
        competitiveness, market_insight, and per-platform competitor data.
    """
    try:
        if sku:
            product = get_product_by_sku(sku)
        ...
        category_products = []
        if sku:
            nearby = get_products_by_category(product_category, limit=50)
            for p in nearby:
                p_sku = p.get('sku', '')
        ...
        return {
            ...
            'category_products': category_products[:15],
...
def _compute_price_recommendation_inline(product: Dict, guardrails: Dict) -> Dict:
    """
    Compute price range, competitor comparison, and demand impact for a single
    product using the data already in memory — avoids redundant DynamoDB lookups.
    """
    sku = product.get('sku', '')
    current_price = float(product.get('price', 0))
    category = product.get('category', 'Uncategorized')
    ...
    # Mock competitor price (variance based on price band)
    if current_price < 500:
        variance = random.uniform(0.90, 1.10)
    elif current_price < 1500:
        variance = random.uniform(0.85, 1.15)
    ...
    if current_price < min_price:
        action = 'INCREASE_PRICE'
        reason = f'Below min margin threshold ({guardrails["min_margin_percent"]}%)'
    elif current_price > max_price:
        action = 'DECREASE_PRICE'
        reason = 'Above competitive threshold'
    elif abs(current_price - optimal_price) / optimal_price < 0.05:
        action = 'MAINTAIN_PRICE'
        reason = 'Price is well-positioned'
    elif current_price < optimal_price:
        ...
        action = 'CONSIDER_DECREASE'
        reason = 'Consider reduction to improve competitiveness'

    price_change = optimal_price - current_price
    price_change_pct = (price_change / current_price * 100) if current_price > 0 else 0

    # Competitiveness
    price_diff_pct = ((current_price - competitor_price) / competitor_price * 100) if competitor_price > 0 else 0
    if price_diff_pct < -5:
    ...
    # Demand / revenue impact (simple elasticity)
    if current_price < 500:
        elasticity = PRICE_ELASTICITY['budget']
    elif current_price < 1500:
        elasticity = PRICE_ELASTICITY['mid_range']
    ...
    demand_change_pct = price_change_pct * (elasticity / 100)
    expected_velocity = max(0, velocity * (1 + demand_change_pct / 100))

    current_revenue = current_price * velocity
    expected_revenue = optimal_price * expected_velocity
    revenue_change_pct = ((expected_revenue - current_revenue) / current_revenue * 100) if current_revenue > 0 else 0

    current_margin_abs = (current_price - cost_price) * velocity
    expected_margin_abs = (optimal_price - cost_price) * expected_velocity
    margin_change_pct = ((expected_margin_abs - current_margin_abs) / current_margin_abs * 100) if current_margin_abs > 0 else 0

    # Priority score
    priority_score = 0
    if action in ['INCREASE_PRICE', 'CONSIDER_INCREASE']:
        priority_score += 30
    if action in ['DECREASE_PRICE', 'CONSIDER_DECREASE']:
        priority_score += 20
    ...
    return {
        ...
        'competitiveness': competitiveness,
        'expected_revenue_change_pct': round(revenue_change_pct, 2),
        'expected_margin_change_pct': round(margin_change_pct, 2),
        'recommendation': action,
...
@tool
def get_price_optimization_recommendations(category: str = None, limit: int = 1000) -> Dict:
    """
    Get prioritized price optimization recommendations for products.

    Args:
        category: Filter by product category (None for all categories)
        limit: Maximum number of recommendations to return (default: 1000)

    Returns:
        Dict with total_recommendations, summary, and ranked recommendations list.
    """
    try:
        if category:
            products = get_products_by_category(category, limit=10000)
        else:
        ...
        for product in products:
            rec = _compute_price_recommendation_inline(product, guardrails)
            if rec:
                recommendations.append(rec)

        recommendations.sort(key=lambda x: x['priority_score'], reverse=True)

        # Apply limit after sorting so the most important ones are kept
        ...
        avg_price_change_pct = sum(r['price_change_pct'] for r in recommendations) / total_products if total_products > 0 else 0
        total_revenue_impact = sum(r['expected_revenue_change_pct'] for r in recommendations) / total_products if total_products > 0 else 0
        ...
        return {
            ...
            'summary': {
                ...
                'total_revenue_impact_pct': round(total_revenue_impact, 2),
            },
...
agent = Agent(
    ...
    tools=[
        calculate_recommended_price_range,
        get_competitor_price_comparison,
        get_price_optimization_recommendations,
    ],
    system_prompt=(
        "You are the HeartKart Pricing Intelligence Agent. Analyze pricing data to provide "
        "actionable recommendations for Valentine's Day product pricing.\n\n"
        "Use your tools to:\n"
        "1. Calculate recommended price ranges using calculate_recommended_price_range\n"
        "2. Compare prices with competitors using get_competitor_price_comparison\n"
        "3. Get optimization recommendations using get_price_optimization_recommendations\n\n"
        "Provide:\n"
        "- Key pricing insights and optimization opportunities\n"
        "- Competitive positioning analysis\n"
        "- Demand impact considerations\n"
        "- Actionable pricing strategy recommendations\n\n"
        "Be concise, data-driven, and focused on maximizing Valentine's Day revenue."
    ),
)
...
def generate_pricing_intelligence_report(
    query: str = None, sku: str = None, category: str = None, analysis_type: str = 'full'
) -> Dict:
    ...
    try:
        ...
        if analysis_type == 'price_range':
            ...
            # Use the product's category for fetching related products
            pr_category = result.get('price_range', {}).get('category', category)
            recs = get_price_optimization_recommendations(category=pr_category, limit=1000)
            if not recs.get('error'):
                result['optimization_recommendations'] = recs
                # Populate related_products for the table in the price_range tab
                related = []
                for rec in recs.get('recommendations', []):
                    ...
                    related.append({
                        ...
                        'optimal_price': rec['recommended_price'],
                        'competitor_price': rec.get('competitor_price', round(rec['current_price'] * 0.98, 2)),
                        'current_margin_percent': rec['current_margin'],
                        'action': rec['recommendation'],
                    })
                result['related_products'] = related[:20]

        elif analysis_type == 'competitor_comparison':
            if sku or category:
                comp = get_competitor_price_comparison(sku=sku, category=category)
                if not comp.get('error'):
                    result['competitor_comparison'] = comp
            # Also include optimization recommendations for all-products competitor view
            recs = get_price_optimization_recommendations(category=category, limit=1000)
            if not recs.get('error'):
                result['optimization_recommendations'] = recs

        elif analysis_type == 'demand_impact':
            if not sku:
                return {'status': 'error', 'error': 'sku required for demand_impact'}
            product = get_product_by_sku(sku)
            if not product:
            ...
            demand = calculate_demand_impact(sku, new_price, current_price)
            if demand.get('error'):
                return {'status': 'error', 'error': demand['error']}
            result['demand_impact'] = demand
            result['price_range'] = price_range
            # Add a list of other products for navigation within the demand tab
            nav_recs = get_price_optimization_recommendations(limit=50)
            if not nav_recs.get('error'):
                result['demand_products'] = [
                    {
                        'sku': rec['sku'],
                        'product_name': rec['product_name'],
                        'current_price': rec['current_price'],
                    ...
                    for rec in nav_recs.get('recommendations', [])[:20]
                    if rec.get('sku') != sku
                ]

        elif analysis_type == 'optimization':
            recommendations = get_price_optimization_recommendations(category=category, limit=1000)
        ...
        else:  # full report — always include optimization recommendations for ALL products
            recommendations = get_price_optimization_recommendations(
            ...
            if sku:
                price_range = calculate_recommended_price_range(sku=sku)
                comp = get_competitor_price_comparison(sku=sku)
                if not price_range.get('error') and not comp.get('error'):
                    current_price = price_range['current_price']
                    optimal_price = price_range['price_range']['optimal_price']
                    demand = calculate_demand_impact(sku, optimal_price, current_price)
                    result['price_range'] = price_range
                    result['competitor_comparison'] = comp
                ...
                comp = get_competitor_price_comparison(category=category)
                result['price_range'] = price_range
                result['competitor_comparison'] = comp
...
@app.entrypoint
def pricing_intelligence_handler(payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    AgentCore Runtime entry point for the Pricing Intelligence Agent.

    Args:
        payload: Input containing query, sku, category, analysis_type

    Returns:
        Dictionary with pricing intelligence report
    """
    print(f"💰 Pricing Intelligence Agent - Received request: {json.dumps(payload, default=str)}")
    ...
    try:
        ...
        result = generate_pricing_intelligence_report(query, sku, category, analysis_type)
        return result

    except Exception as e:
        import traceback
        print(f"Error in pricing intelligence handler: {e}")
        traceback.print_exc()
        return {'status': 'error', 'error': str(e), 'traceback': traceback.format_exc()}

...
It uses AWS CodeBuild to build the Docker container.
"""
...
def main():
    ...
    print("=" * 70)
    print("🚀 Deploying Replenishment Planner Agent to AWS Bedrock AgentCore Runtime")
    print("=" * 70)
    print(f"\n📍 Region: {region}")
    print(f"📍 Account: {boto_session.client('sts').get_caller_identity()['Account']}")
    
    agentcore_runtime = Runtime()
    agent_name = "heartkart_replenishment_planner"
    ...
    print("=" * 70)
    print("   This will:")
    print("   1. Build Docker container using AWS CodeBuild")
    ...
    try:
        ...
        if isinstance(invoke_response, dict):
            status_val = invoke_response.get('status', 'unknown')
            summary = invoke_response.get('summary', {})
            print(f"   Status: {status_val}")
            if summary:
                print(f"   Total products needing reorder: {summary.get('total_products_needing_reorder', 'N/A')}")
                print(f"   Critical urgency items: {summary.get('critical_urgency', 'N/A')}")
                print(f"   Total estimated cost: {summary.get('total_estimated_cost', 'N/A')}")
        else:

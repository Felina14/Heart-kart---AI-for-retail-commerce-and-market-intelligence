...
Detects anomalies in sales patterns and inventory behavior
"""
...
def detect_anomalies(sales_data: Dict, threshold: float = 1.5) -> List[Dict]:
    """
    Detect anomalies using simple comparison of recent vs historical averages
...
def enrich_with_product_data(anomalies: List[Dict]) -> List[Dict]:
    """Add product details from DynamoDB valentines-products table."""
    enriched = []
...
def investigate_exceptions(days: int = 30, threshold: float = 2.0) -> Dict:
    """
    Main function to investigate exceptions
    ...
    Returns:
        Dict with anomalies, insights, and summary
    """
...
if __name__ == '__main__':
    print("=" * 70)
    print("🔍 EXCEPTION INVESTIGATOR - Anomaly Detection")
    print("=" * 70)
    ...
    if result['anomalies']:
        ...
        for i, anomaly in enumerate(result['anomalies'][:5], 1):
            print(f"\n{i}. {anomaly['name']} ({anomaly['sku']})")
            print(f"   Type: {anomaly['anomaly_type']} | Severity: {anomaly['severity']}")
            print(f"   Z-Score: {anomaly['z_score']}")
            print(f"   Recent Avg: {anomaly['recent_avg_daily']}/day | Historical: {anomaly['historical_avg_daily']}/day")
            print(f"   Stock: {anomaly['stock_quantity']} | Price: ${anomaly['price']}")
